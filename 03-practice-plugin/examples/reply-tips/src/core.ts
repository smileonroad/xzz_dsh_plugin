 ...
 * reply-tips — 纯函数核心
 *
 * 动态版运行体（`dynamic/code.host.js` + `dynamic/code.client.js`）与静态包双端共用这一层：
 * Host 侧的回扫取数 / 规则兜底 / 模型输出解析 / 提示词构建，以及开关文件的读写。
 * 宿主耦合的部分（React 组件、slots 注册、ctx 服务访问、Remote 调用循环）留在两侧入口，
 * 不在这里静态表达。插件名与 inject 由各侧入口自己声明，本文件不导出插件形状的导出项。
 *
 * 纯函数部分用 `tests/reply-tips.spec.ts` 在 Node 下钉住。
 */

export const FILE_NAME = '.reply-tips.json'

/** 生成失败原因（diag.reason 的取值，Client 据此显示回退文案）。 */
