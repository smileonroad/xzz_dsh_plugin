...
上面的清单是 dsh 仓库内 `packages/` 的标准包。同一套包形状可以**独立分发**，装进任何 profile；`examples/grill-send-button/` 实战（提案 `meta/proposals/2026-09-09-grill-send-standard-package.md`）按本流程走通并实测。
