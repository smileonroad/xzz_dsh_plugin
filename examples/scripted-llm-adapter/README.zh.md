...
这条链上只有 `stream()` 是必须实现的，另外三个（`providerInfo` / `listModels` / `resolveModel`）都有默认实现，本项目按需要覆写，用来演示「目录是建议性的、能力要如实声明」这两条约定。

`prepareCall` 的默认实现把「这个模型有哪些能力」和「这一次请求从哪个入口发出去」合成一次问完，答案绑在一起用。有些适配器的模型目录是运行时现拉的，两次操作之间配置可能已经换了，绑在一起就不会拿着旧模型的参数去请求新模型。我们的离线模型用不上这层保护，但默认实现顺手给了。
...
`resolveModel()` 里声明的东西不是写给文档看的，框架真的照着它拦人。适配器声明了思考档位（reasoning，比如 off、low、high）之后，这些档位会按它给的顺序原样透出，包括「关掉思考」那一档；调用方点名一个没声明的档位，`LlmRuntime` 在调用 `stream()` **之前**就拒绝，测试用的证据是适配器一次都没被调到。调用方什么都不说时，落到适配器声明的默认档位。

注册的规矩也值得记住。同一个路由只能有一个适配器，重复注册会抛 `DUPLICATE_ADAPTER`；一次注册多个路由要么全成要么全败；注册返回的句柄除了能注销，还有一个 `replace()` 可以整体换掉路由，换之前先全部校验，换的时候是一次同步操作，中途没有空窗；句柄释放之后再 `replace` 会抛 `REGISTRATION_DISPOSED`。注册本身是副作用，跟着插件生命周期自动回收，所以 HMR 安全。
...
**第三组，能力声明。** 声明了哪些 reasoning 档位就得守规矩。调用方点名要一个没声明的档位时，`stream()` 一次都不该被调用；不点名时应该落到声明里的默认值；目录里没列的模型 id 照样能用，因为目录只用来展示。
