...
> 相关代码：`packages/sdk/`（protocol / client / server）、[scripts/jsonrpc-mini-client.mjs](../scripts/jsonrpc-mini-client.mjs)、[scripts/jsonrpc.cordis.yml](../scripts/jsonrpc.cordis.yml)
...
[scripts/jsonrpc-mini-client.mjs](../scripts/jsonrpc-mini-client.mjs) 是零依赖的最小客户端：

```bash
node scripts/jsonrpc-mini-client.mjs "要问的话" 模型名
```

例如：

```bash
node scripts/jsonrpc-mini-client.mjs "你好，用一句话介绍你自己。" deepseek-v4-flash
```

它本地 spawn `packages/examples/jsonrpc-scripts/src/bin.ts`（dsh-jsonrpc-agent），配置指向 [scripts/jsonrpc.cordis.yml](../scripts/jsonrpc.cordis.yml)（minimal 组合 + `thinking: disabled`）。
...
1. **url（网关地址）**：`DEEPSEEK_BASE_URL` 由启动环境提供（bootstrap-only，不能放 `.env`），[scripts/jsonrpc-mini-client.mjs](../scripts/jsonrpc-mini-client.mjs) 从环境变量读取后透传给子进程。换网关改环境变量即可。
...
3. **模型名称**：走 `initialize` 的协议参数（`provider` + `model`），client 第二个命令行参数直接传：
   ...
   ```bash
   node scripts/jsonrpc-mini-client.mjs "要问的话" deepseek-v4-flash
   ```
   ...
   相比 headless（`--patch` 覆盖 `agent-default-model`）和 acp（改 `cordis.yml` 的 `model` 字段），jsonrpc **换模型完全不用改配置树**——这是它「极简、程序化」定位的直接体现。模型 id 仍需网关认识（[scripts/jsonrpc.cordis.yml](../scripts/jsonrpc.cordis.yml) 的 `llm-deepseek.models` 列表列出网关认识的 id，原样透传）。
...
修复：在 `llm-deepseek` 配置里加 `thinking: disabled`（见 [scripts/jsonrpc.cordis.yml](../scripts/jsonrpc.cordis.yml)）。但注意：即使 disabled，模型对难题仍会先产生一小段 reasoning——这是网关行为，不是 DSH 问题。
