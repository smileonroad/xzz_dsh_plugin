# 提案：gatehouse-demo —— 传达室（审批的应答者）

- 日期：2026-08-26
- 状态：待确认（探索完成，开发前需确认）
- 对应官方指引：deepseek-harness `docs/subsystems/approval.md`（本仓库未收副本）+ cordis-tutorial 04-events（教程点名句：「`approval/request` lets a policy answer instead of the user」）+ `packages/interaction/user-approval` 源码
- 系列位置：第 7 个实战；承接 events-demo / tea-shop-demo 的事件话题——前两个实战用自声明事件练 waterfall 的观察者/决策者纪律，本实战把同一套纪律用到**真实 harness 的 `approval/request` 上**，练 seam 的第三个角色：应答者
...
| 传达室大爷先看名单 | `approval/request` waterfall 的应答者链 |
...
4. 挂载顺序是真实约束，也是教学金矿：patch 层序（base → web-app → profile 自身 patch → `--patch` overlay）让 overlay 挂的应答者排在 UI answerer **之后**；`ctx.on` 的 `prepend` 选项是文档化的插队机制（user-approval 源码注释明确讨论过 prepend 与 `'never'` 策略的关系）。而 `'never'` 策略在服务层、分发**前**判定，prepend 也绕不过——「策略的层次」正好借传达室讲：传达室大爷再厉害，也拦不住锁门的决定。

## 探索摘要（关键事实）

- `ApprovalOutcome`：`'allowed-once' | 'rejected' | 'cancelled' | 'unavailable'`，闭集、fail-closed；`allowed-once` 是唯一放行。
- `ctx.approval.request(req)`：要求**开着的 turn**（审计对必须被 turn 包住）；先 append `approval/asked`，分发 waterfall，再 append `approval/decided`；无应答者 → `'unavailable'`；应答者 throw → `'unavailable'`；非词汇返回值 → 规范化为 `'unavailable'`。
- waterfall 分发是 scope-filtered（`scopeTarget(agent)`）；应答者自行决定认领/委托。
...
| 插件 | 角色 | 教学点 |
|---|---|---|
| `gatehouse-keeper` | 应答者 | Config `{ allow: string[], deny: string[], prepend: boolean }`（Schemastery，csv-query 套路）；`allow` 命中 → `'allowed-once'`，`deny` 命中 → `'rejected'`，其余 `next()` 委托；`prepend: true` 时注册到链首（自动规则放第一道门）；`inject: ['approval']` |
...
- fail-closed：无应答者 → `'unavailable'`；应答者 throw → `'unavailable'`；非词汇返回值 → 规范化为 `'unavailable'`
...
- 不做 scope 过滤的 agent 专属应答者（`this: Scoped<…>` 的用法，后续候选）
