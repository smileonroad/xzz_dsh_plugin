...
> 相关代码：逐篇深入见 [README](../README.md) 的链接；运行器都在 [scripts/](../scripts/) 下
...
- **同一套凭证与网关机制**。key 走根 `.env` 的 `DEEPSEEK_API_KEY`；网关 `DEEPSEEK_BASE_URL` 是 bootstrap-only，只能由启动环境提供（app-boot 拒收 `.env` 里声明它），所以每个 运行器都用 `env: { ...process.env, DEEPSEEK_BASE_URL }` 显式传入。
