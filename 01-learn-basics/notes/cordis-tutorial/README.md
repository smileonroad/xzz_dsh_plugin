...
> **进度**：8 章全部已落盘。实验源码 **154 个文件**：大部分逐字来自文档代码块，`cordis.yml` 按代码补全，另有 **22 个文件** 是「补全件」（按正文 + 同模块源码推得，首行带标记），**60 个实验已实跑校验**，清单与比对结果见 [examples/cordis-tutorial/README.md](../../examples/cordis-tutorial/README.md)。
>
> ⚠️ **实测与原文不符的地方已在正文上标了 14 处「实测更新」**（第 1、3、5、6 章）：主要是 harness 不再把加载失败抛出来，那些「崩溃 + 退出码 1」的记录现在是**零输出 + 退出码 0**。原文保留，只在下面追加标注 —— 看失败信息用 `node --import tsx ../_diag.ts`。
