...
**只有 `stream()` 是必须实现的**，另外三个（`providerInfo` / `listModels` / `resolveModel`）都有默认实现。本项目按需要覆写它们，演示两条约定，**目录是建议性的，能力要如实声明**。

`prepareCall` 把「这个模型有哪些能力」和「这一次请求从哪个入口发出去」合成一次问完，答案绑在一起用。有些适配器的模型目录是运行时现拉的，两次操作之间配置可能已经换了，**绑在一起就不会拿着旧模型的参数去请求新模型**。我们的离线模型用不上这层保护，默认实现顺手给了。
...
**`resolveModel()` 里声明的东西不是写给文档看的，框架真的照着它拦人。** 声明了思考强度（reasoning，比如 off、low、high）之后，这些强度按适配器给的顺序原样透出，包括「关掉思考」那一档；调用方点名一个没声明的强度，`LlmRuntime` 在调用 `stream()` **之前**就拒绝，测试的证据是适配器一次都没被调到。什么都不说时落到声明的默认强度。

注册的规矩也值得记住。

- 同一个路由只能有一个适配器，重复注册抛 `DUPLICATE_ADAPTER`。
- 一次注册多个路由，要么全成，要么全败。
- 句柄能注销，也能用 `replace()` 整体换掉路由；换之前先全量校验，换是一次同步操作，中途没有空窗；释放之后再换抛 `REGISTRATION_DISPOSED`。
- 注册是副作用，跟着插件生命周期自动回收，所以 HMR 安全。

### 拦截层：不调模型也能回答

图里第 ① 步是一条瀑布，插件可以在模型调用外面包一层，甚至根本不往下传。本示例用它做了一个敏感词门禁（`src/guard.ts`）：输入带「权限」「密码」这类词就直接回一句「非法内容，请重新输入。」，**适配器一次都不会被调用**。

**策略没有写进适配器，因为策略不该长在提供方身上。** 换个真模型，适配器就换了，而门禁与用哪个模型无关，「模型调用之前」才是它的位置。
...
- 判断对象是**最后一条人说的话**，工具结果和 harness 注入的 user 消息都不算，`lastUserText()` 已经处理过这两个坑。
- `options.purpose` 非空的是压缩、起标题这类后台调用，必须直接放行，否则会打断它们。
- 拒绝时返回的必须是一段**合规**分片流，块启停配对加 `finish` 收尾，因为它和适配器的产出走同一条校验；测试里特意把它挂在包不变量下跑了一遍。没有模型调用，所以不报 `usage`。
...
- **不解析厂商协议。** 想练 HTTP 请求映射、`attributionHeaders()`、SSE 解析与重试分类，把离线模型换成对着 `packages/test-support/llm-mock-server`（OpenAI 兼容的故障服务器）写的 HTTP 适配器就行。
- **门禁只拦「人说的话」，原文仍会落盘。** 若合规要求原文不进历史，得改用 `agent/pre-step` 的 `{ kind: 'reject' }`，并自行确认 inbox 认领事件里是否仍带原文。
- **词表是子串匹配，会误伤。** 「权限管理」也会被拦。词表走 `Config`，可以在 patch 里改（见 `cordis.patch.yml`），也可以换成正则或加白名单。
- **选择器可见性只在 host 侧验证过。** 测试直接调用 web 用的那个 `buildModelCatalog`，没有自动化的浏览器点击验证。
- **离线模型不做真实推理。** `usage` 按消息条数与字符数推算，别拿它做计量实验。

## 怎么分发

本示例按教学示例交付，不带 `package.json`，不参与 bundle 通道。要做成可安装包（`package.json` 声明 `dsh.bundle.patch`、自带 `cordis.patch.yml`、预构建 `lib/`）的完整流程见 [docs/plugin-package.md](../../notes/plugin-package.md)，现成的两个样板是 [grill-send-button](../grill-send-button/) 与 [reply-tips](../reply-tips/)。
