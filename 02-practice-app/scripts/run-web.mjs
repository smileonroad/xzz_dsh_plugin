 ...
 * overlays.
 *
 *   node 02-practice-app/scripts/run-web.mjs                    # web-cordis demo, port 3081
 *   node 02-practice-app/scripts/run-web.mjs --patch <file>     # extra overlays (repeatable)
 *   node 02-practice-app/scripts/run-web.mjs --no-cordis        # plain web, no tool-cordis
 *
...
for (let i = 2; i < process.argv.length; i++) {
  ...
  if (arg === '--patch') {
    ...
    if (file === undefined) {
      console.error('usage: node 02-practice-app/scripts/run-web.mjs [--patch <file> ...] [--no-cordis]')
      process.exit(2)
    }
...
console.log('[02-practice-app/scripts/run-web.mjs] 浏览器打开 http://127.0.0.1:3081 （Ctrl+C 退出）')
