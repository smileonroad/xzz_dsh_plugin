...
> 相关代码：`apps/cli`（dsh CLI）、[scripts/run-headless.mjs](../scripts/run-headless.mjs)
...
运行器脚本 [scripts/run-headless.mjs](../scripts/run-headless.mjs) 把整条命令封装好了，一行就跑（在 deepseek-harness 根目录，脚本已同步到其 `scripts/`）：

```bash
node scripts/run-headless.mjs "任务描述"
```

比如我当初验证连通性跑的是：

```bash
node scripts/run-headless.mjs "用一句话回答：DSH headless 模式是什么？"
```

换模型用 `--patch` 追加 overlay：

```bash
node scripts/run-headless.mjs --patch scripts/model.patch.yml "任务描述"
```
...
换模型用的 [scripts/model.patch.yml](../scripts/model.patch.yml) 也很简单，就是一个覆盖 `agent-default-model` 配置的 overlay：
...
1. **url（网关地址）**：`DEEPSEEK_BASE_URL` 由启动环境提供（bootstrap-only，不能放 `.env`），[scripts/run-headless.mjs](../scripts/run-headless.mjs) 从环境变量读取后透传给子进程。换网关改环境变量即可。
...
3. **模型名称**：headless 的默认模型来自 base bundle 的 `agent-default-model`，用 `--patch` 覆盖它：
   ...
   [scripts/model.patch.yml](../scripts/model.patch.yml) 内容就是覆盖 `agent-default-model` 的 config（模型 id 原样透传，网关认识即可）：
