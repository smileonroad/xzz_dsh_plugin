...
动态流程证明了行为，但没发布任何东西。想把这按钮交给别的 profile，就要按 client-plugin cookbook 把它提升成标准 Client 插件包，`src/client/index.ts` 入口加 `dsh.client` manifest 行，rebuild bundle 再重启 profile。发布版 web HMR 默认禁用，新插件必须重启进程才生效。这一步和 csv-query 那篇的分发流程是同一个套路，只是目标从 Host bundle 换成了 web bundle。

再往后，Client 侧值得练的还很多。往输入栏加按钮只是槽位系统最浅的一层，session 作用域、keyed 槽、composer 之外还有什么 standard props 可用，都还没摸。
